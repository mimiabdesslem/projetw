...
$listeEmploye1=$ClientC->afficherClient();
}
else{
	
	$listeEmployes=$ClientC->rechercheClient($_GET['recherche']);
	$listeEmploye1=$ClientC->afficherClient();
...
foreach($listeEmploye1 as $rows){
  ...
  ?>
  <option value="<?PHP echo $rows['Telephone']; ?>">
   <?PHP
}
?>
</datalist>
...
foreach($listeEmployes as $row){
  ...
  <td><a href="modifierClient.php?id=<?PHP echo $row['Telephone']; ?>" style="text-decorations:none; color:white;"  class="btn btn-warning">
  Modifier</a></td>
  </tr>
  <?PHP
}
